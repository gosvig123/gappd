...
// CleanupMeeting removes at most 100 expired real copies per invocation. Scheduling is external.
func CleanupMeeting(ctx context.Context, conn *pgx.Conn) (int64, error) {
	var safe bool
	err := conn.QueryRow(ctx, `SELECT current_user='gappd_meeting_cleanup' AND NOT rolsuper AND NOT rolbypassrls
 ...
 FROM pg_roles WHERE rolname=current_user`).Scan(&safe)
	...
	count, err := meetingCleanupBatch(ctx, tx)
	if err != nil {
		return 0, err
	}
...
func meetingCleanupBatch(ctx context.Context, tx pgx.Tx) (int64, error) {
 ...
 SELECT l.owner_id,l.id FROM meeting_lifecycle l JOIN cloud_meetings m USING(owner_id,id)
 WHERE l.expires_at<=statement_timestamp() ORDER BY l.expires_at,l.id LIMIT 100 FOR UPDATE OF l SKIP LOCKED
 ), marked AS (
 UPDATE meeting_lifecycle l SET deleted_at=coalesce(l.deleted_at,statement_timestamp()) FROM candidates c
 WHERE l.owner_id=c.owner_id AND l.id=c.id RETURNING l.owner_id,l.id
	...
	deleted, err := tx.Exec(ctx, `DELETE FROM cloud_meetings WHERE (owner_id,id) IN (
 SELECT m.owner_id,m.id FROM cloud_meetings m JOIN meeting_lifecycle l USING(owner_id,id)
