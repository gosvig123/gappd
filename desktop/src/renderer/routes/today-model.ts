...
export function readyState(canStart: boolean, devices: Device[], status: RecordingStatus): CaptureReadiness {
  ...
  if (status === RECORDING_STOPPING) return { detail: 'Stopping recording. Notes start after audio saves.' }
  if (status === RECORDING_PROCESSING) return { detail: 'Meeting saved. Finishing notes locally. Keep app open.' }
