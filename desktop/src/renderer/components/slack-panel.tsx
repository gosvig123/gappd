...
export function SlackPanel({ slack }: { slack: SlackConnectionController }) {
  ...
  return (
    <Card className="settings-section">
      <div className="settings-section-head"><div><h2>Slack</h2><p>Connect Gappd to your Slack workspace. Sending messages is not available yet.</p></div><StatusPill tone={statusTone(slack, expired)}>{statusLabel(slack, expired)}</StatusPill></div>
...
function connectionNote(slack: SlackConnectionController, expired: boolean): string {
  ...
  return 'Slack is connected. Gappd does not send messages yet.'
}
