...
const GENERATE_HINT = 'Checking and syncing Calendar history when needed, then preparing from local Meetings. Long histories use up to 96 model requests, with at most four in parallel and can take up to 20 minutes. Provider usage charges may apply…'
...
export function MeetingAgendaDraftPanel({ draftKey, sourceId, canGenerate, knownMeetingIds, onOpenMeeting, onOpenSettings }: PanelProps) {
  ...
  return (
    <MeetingAgendaDraftView
      ...
      onGenerate={() => void controller.generate()} onChange={(next) => controller.setTopics(next.items.map((item) => item.topic))}
...
export function MeetingAgendaDraftView({ draft, busy, error, onGenerate, onChange, onOpenMeeting, onOpenSettings, canGenerate = true, knownMeetingIds, saveState, loadingSaved, onRetrySave, onReloadSaved, onDeleteSaved }: ViewProps) {
  ...
  const blocked = Boolean(draft?.items.length && !retry)
  const generate = () => {
  ...
  return (
    <div className="meeting-agenda-draft">
      <Button className="compact-action" disabled={busy || !canGenerate || blocked} onClick={generate}>{busy ? 'Generating agenda…' : retry ? 'Generate again…' : 'Generate agenda'}</Button>
...
function DraftNotice({ draft }: { draft: MeetingAgendaDraft }) {
  ...
  if (!draft.items.length) return <p role="status">No supported follow-up topics found in {draft.sources.length} matched Meetings.{draft.historyIncomplete ? ` ${draft.historyWarning ?? HISTORY_INCOMPLETE_WARNING}` : ''}</p>
  return null
}
...
function AgendaTopics({ draft, busy, onChange, onOpenMeeting, knownMeetingIds }: { draft: MeetingAgendaDraft; busy: boolean; onChange: (draft: MeetingAgendaDraft) => void; onOpenMeeting: (id: string) => void; knownMeetingIds?: ReadonlySet<string> }) {
  return (
    <>
      <p>Saved on this Mac for this event only. Nothing is shared or added to Calendar. Based on up to 12 matched Meetings; history may be incomplete. Confirm current status before use.</p>
      ...
      {draft.items.map((item, index) => <AgendaTopic key={`${item.sourceId}:${index}`} index={index} topic={item.topic} quote={item.quote} readOnly={busy} onChange={(topic) => onChange({ ...draft, items: draft.items.map((value, position) => position === index ? { ...value, topic } : value) })} sourceId={item.sourceId} sourceLabel={draft.sources.find((source) => source.id === item.sourceId)?.title ?? item.sourceId} removed={Boolean(knownMeetingIds && !knownMeetingIds.has(item.sourceId))} onOpenMeeting={onOpenMeeting} />)}
    </>
  )
}

function AgendaTopic({ index, topic, quote, readOnly, onChange, sourceId, sourceLabel, removed, onOpenMeeting }: { index: number; topic: string; quote: string; readOnly: boolean; onChange: (topic: string) => void; sourceId: string; sourceLabel: string; removed: boolean; onOpenMeeting: (id: string) => void }) {
  return (
    <div>
      <label>Topic {index + 1}<textarea readOnly={readOnly} value={topic} onChange={(event) => onChange(event.target.value)} /></label>
      <blockquote>{quote}</blockquote>
      {removed ? <p role="status">Source Meeting removed: {sourceLabel}</p> : <Button className="compact-action" onClick={() => onOpenMeeting(sourceId)}>Open Meeting: {sourceLabel}</Button>}
    </div>
  )
}
