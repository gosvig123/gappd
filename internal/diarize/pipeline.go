...
func Transform(in Input) (Output, error) {
	...
	assignments, count, coverage := align(in.Phrases, stitch(in.Windows))
	...
	for wi, window := range windows {
		...
		for _, span := range window.Spans {
			...
			if end > start {
				...
				out = append(out, stitchedSpan{match.speaker, start, end, confidence})
			}
		}
	}
	return out
...
func matchWindow(window WindowReport, clusters []LocalCluster, globals map[int]*globalCluster, previous []stitchedSpan) map[string]clusterMatch {
	...
	for _, edge := range edges {
		...
		continuity := math.Min(similarity, .5+.5*clamp((edge.score-second)/.1))
		matches[edge.local], used[edge.speaker] = clusterMatch{edge.speaker, continuity}, true
	}
	return matches
}
...
func align(phrases []Phrase, spans []stitchedSpan) ([]db.SpeakerProjectionAssignment, int, float64) {
	...
	for speaker, indexes := range rawBySpeaker {
		value, _ := metricFor(indexes, spans)
		rawSeconds += value
	...
	if len(active) == 0 && eligible > 0 {
		top, selected = strongestTurn(spans, bySpeaker)
	...
	for _, phrase := range phrases {
		...
		if totalSupport > 0 && share < WinnerShareThreshold {
			...
			confidence := math.Min(winnerSupport/raw[winner], math.Min(share, covered))
