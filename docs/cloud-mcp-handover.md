...
| Grant revocation | Implemented end to end. `POST /revoke` cuts one client, or every client with `*`, and Settings offers it with a confirmation naming the client. The check runs on every request with a 30-second in-process cache, so a revoked client can work for up to 30 seconds. A revocation is permanent, and there is still no client inventory, so the user must supply the client id. |
