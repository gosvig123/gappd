...
| Grant revocation | Implemented end to end. `POST /revoke` cuts one client, or every client with `*`, and Settings offers it with a confirmation naming the client. The check runs on every request with a 30-second in-process cache, so a revoked client can work for up to 30 seconds. A revocation is permanent. `POST /clients` lists the clients seen for the account, most recently used first, and Settings offers them as a pick-list with a typed id as a fallback. |
| Device registration | Server side implemented: `POST /device` registers an Ed25519 public key and every write needs a signature over the method, path, device, generation and body digest. The desktop does not generate or sign yet, so uploads are refused until it does. |
| Account generations | Implemented end to end. `POST /delete-all` erases every copy, bars each identity, clears the local queue and closes uploads under a new generation; `POST /consent` reopens them and the app remembers the generation the server issued, so a device holding the old one is still refused. Settings offers both, each with its own confirmation. |
